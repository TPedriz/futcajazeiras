-- =====================================================================
-- Gamificação: conquistas, XP e níveis
-- Tabelas, helpers de XP, triggers automáticos, destaques (máx. 3) e seed.
-- =====================================================================
...
-- Máximo de 3 conquistas em destaque por usuário.
...
-- XP cumulativo necessário para alcançar um nível (100 * n * (n-1) / 2):
...
-- Nível correspondente a um total de XP (inverso da função acima).
CREATE OR REPLACE FUNCTION public.nivel_para_xp(xp integer)
RETURNS integer LANGUAGE sql IMMUTABLE AS $$
  SELECT floor((1 + sqrt(1 + 8.0 * GREATEST(xp, 0) / 100)) / 2)::integer;
$$;

-- Concede XP a um usuário, recalcula o nível e verifica conquistas.
...
-- ============ 5) Verificação/desbloqueio de conquistas ============
...
BEGIN
  ...
  -- Presenças do próprio jogador (compareceu) + presenças dele como convidado
  -- (convidado_user_id aponta para a conta dele). Presença de convidado que
  -- ele levou não conta para ele.
...
-- Presença confirmada (compareceu = TRUE) → +10 XP (presença própria) ou +10 XP
-- para o convidado com conta, quando a presença for de convidado.
...
-- Gols (+5 cada) e assistências (+3 cada) lançados nas estatísticas do baba.
